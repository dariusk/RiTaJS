    ...
    RiMarkov.prototype = {
        ...
        generateSentences : function(num) {
		    ...
		    if (!this.recognizeSentences) 
		    	err("Illegal state: attempt to call generateSentences()"
		      		+ " after setting generateSentences=false");
		                        
		    var s, result = [], counter=0, totalTries=0, wordsInSentence=1;
		 ...
		 */ 
		 _loadSentences : function(sentences, multiplier) {
			...
			var allWords = [], printIgnoredText = true;

			// do the cleaning/splitting first ---------------------

			for (var i = 0; i < sentences.length; i++) {
				
				var sentence = this._clean(sentences[i]);
			...
			for (var i = 0; i < words.length; i++) {
				...
				toAdd = [];
				var nFactor = this.getN();
				for (var j = 0; j < nFactor; j++) {
					if ((i + j) < words.length)
						toAdd[j] = words[i + j];
				}
